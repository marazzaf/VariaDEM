...
def connectivity_graph(mesh_, d_): #Contains all necessary mesh information
    ...
    for f in facets(mesh_):
        ...
        if len(aux_bis) == 2: #add the link between two cell dofs
            ...
            #adding edge
            G.add_edge(aux_bis[0],aux_bis[1], num=num_global_ddl_facet[0] // d_, dof_CR=num_global_ddl_facet, measure=area, barycentre=bary, normal=normal, vertices=vert, pen_factor=pen_factor[num_global_ddl_facet[0] // d_], breakable=True)
